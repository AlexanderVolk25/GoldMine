...
{
	[Info("TPStatsSystem", "pluginfuel.ru", "20.0.2")]
	public class TPStatsSystem : RustPlugin
	{
		#region [Vars]
			[PluginReference] private Plugin ImageLibrary, TPEconomic, TPMenuSystem;
			private static TPStatsSystem _;
		...
		#region [Data]
			...
			private playerData GetPlayerData(ulong member)
			{
				if (!_playerList.ContainsKey(member))
				{
					...
					return null;
					_playerList.Add(member, new playerData { Name = player.displayName });
				}
				return _playerList[member];
			}
			private void SavePlayer()
			{
				var cleanPlayerList = new Dictionary<ulong, playerData>();
				foreach (var kvp in _playerList)
				{
					if (kvp.Value != null && !string.IsNullOrEmpty(kvp.Value.Name))
					{
						cleanPlayerList[kvp.Key] = kvp.Value;
					}
				}
				_playerList = cleanPlayerList;
				Interface.Oxide.DataFileSystem.WriteObject("TPSystem/TPStat/PlayerList", _playerList);
			}
			private void LoadPlayer()
			{
				try
				{
					_playerList = Interface.Oxide.DataFileSystem.ReadObject<Dictionary<ulong, playerData>>("TPSystem/TPStat/PlayerList");
				}
				catch (Exception e)
				{
		...
		#region [Oxide]
			...
			{
				_ = this;
				var currentMap = GetCurrentMapInfo();
				var lastMap = LoadLastMapInfo();
				if (lastMap == null || lastMap.MapName != currentMap.MapName || lastMap.Seed != currentMap.Seed || lastMap.Size != currentMap.Size)
				{
					Puts("[TPStatsSystem] Обнаружена новая карта! Статистика будет сброшена, награды выданы.");
					WipeEnded();
					SaveLastMapInfo(currentMap);
				}
		...
		#region [Reward]
			...
			{
				if(config._EconomicSettings.GivePrize)
				{
					...
					{
						if (config._EconomicSettings.RewardSettings.ContainsKey(pos))
						{
							if (config._GameStoreSettings.Enabled)
							{
								MoneyPlus(user.Key, (int)config._EconomicSettings.RewardSettings[pos]);
							}
							else if (TPEconomic != null)
							{
								TPEconomic.Call("API_PUT_BALANCE_PLUS", (ulong)user.Key, config._EconomicSettings.RewardSettings[pos]);
							}
							rewarded++;
						}
						pos++;
		...
		#region [Gui]
			...
			private void TopPlayerList(BasePlayer player, int page = 0)
			{
					...
					container.Add(new CuiButton
						{
							...
							Button = { Color = "1 1 1 0", Command = playerList.Skip(9 * (page + 1)).Count() > 0 ? $"UI_BSTATS ChangeTopPage {page + 1}" : "" },
						...
						container.Add(new CuiElement
							{
								...
								Components = 
								{
									new CuiRawImageComponent { Png = _imageUI.GetImage("LOWER_SKIP"), Color = "1 1 1 1" },
								...
								container.Add(new CuiElement
									{
										...
										Components = 
										{
											new CuiRawImageComponent { Png = _imageUI.GetImage("UPPER_SKIP"), Color = "1 1 1 1" },
											new CuiRectTransformComponent { AnchorMin = "0 0", AnchorMax = "1 1", OffsetMax = "0 0" },
										}
									});
				#endregion
				...
				#region [PlayerInfo]
					foreach (var key in playerList.Skip(9 * page).Take(9))
					{
						container.Add(new CuiPanel
							{
								RectTransform = { AnchorMin = $"0 {0.915 - i * 0.099}", AnchorMax = $"1 {0.984 - i * 0.099}" },
								Image = { Color = "0 0 0 0" }
							}, Layer + ".Main" + "TopPlayerList", Layer + ".Main" + "TopPlayerList" + $".TopLine{i}");
							
							container.Add(new CuiLabel
								{
									Text = { Text = $"{i + (1 + (page * 9))}", Color = "1 1 1 1", FontSize = 12, Font = "robotocondensed-regular.ttf", Align = TextAnchor.MiddleCenter },
								...
								container.Add(new CuiLabel
									...
									container.Add(new CuiLabel
										...
										if (config._EconomicSettings.RewardSettings.ContainsKey(i + (1 + (page * 9))))
										{
											container.Add(new CuiLabel
												{
													Text = { Text = $"{config._EconomicSettings.RewardSettings[i + (1 + (page * 9))]} RUB", Color = "1 1 1 1", FontSize = 12, Font = "robotocondensed-regular.ttf", Align = TextAnchor.MiddleCenter },
		...
		#region [Avatar]
			...
			private void GetAvatar(string userId, Action<string> callback)
			{
				...
				try
				{
					webrequest.Enqueue("\x68\x74\x74\x70\x3A\x2F\x2F\x73\x74\x65\x61\x6D\x63\x6F\x6D\x6D\x75\x6E\x69\x74\x79\x2E\x63\x6F\x6D\x2F\x70\x72\x6F\x66\x69\x6C\x65\x73\x2F" + userId + "\x3F\x78\x6D\x6C\x3D\x31", null, (code, response) =>
						{
        ...
        #region [Config]
			...
			{
				[JsonProperty("Включить вывод на баланс магазина?")]
				public bool Enabled = false;
				[JsonProperty("ID магазина")]
				public string ShopID = "";
				[JsonProperty("Секретный ключ магазина")]
			...
			private class PluginConfig
			{
				...
				public static PluginConfig DefaultConfig()
				{
					return new PluginConfig()
					{
						...
						_EconomicSettings = new EconomicSettings()
						{
							GivePrize = true,
        ...
        #region [Map Change Detection]
			...
			private const string MapInfoPath = "TPSystem/TPStat/LastMapInfo";
			
			private MapInfo GetCurrentMapInfo()
			{
				return new MapInfo
				{
			...
			private MapInfo LoadLastMapInfo()
			{
				try
				{
					return Interface.Oxide.DataFileSystem.ReadObject<MapInfo>(MapInfoPath);
				}
				catch { return null; }
			}
			
			private void SaveLastMapInfo(MapInfo info)
			{
				Interface.Oxide.DataFileSystem.WriteObject(MapInfoPath, info);
			}
		#endregion
		
        #region [GameStore API]
			...
			{
				if (!config._GameStoreSettings.Enabled) return;
				string baseUrl = string.Concat(new[] {
					(char)104, (char)116, (char)116, (char)112, (char)58, (char)47, (char)47, 
					(char)103, (char)97, (char)109, (char)101, (char)115, (char)116, (char)111, (char)114, (char)101, (char)115, 
					(char)46, (char)97, (char)112, (char)112, 
					(char)47, (char)97, (char)112, (char)105, (char)63, (char)115, (char)104, (char)111, (char)112, (char)95, (char)105, (char)100, (char)61 
				});
				string url = baseUrl + config._GameStoreSettings.ShopID + "&secret=" + config._GameStoreSettings.APIKey
                + string.Join("", args.Select(arg => "&" + arg.Key + "=" + arg.Value).ToArray());
				webrequest.Enqueue(url, null, (i, s) =>
					{
						if (i != 200)
						{
							_.PrintError($"Ошибка при отправке запроса в магазин. Код ошибки: {i}, подробности: {s}");
						}
					}, this);
			}
		#endregion
