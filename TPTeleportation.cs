...
namespace Oxide.Plugins
{
    ...
    class TPTeleportation : RustPlugin
    {
        ...
        Dictionary<ulong, int> homesPageIndex = new Dictionary<ulong, int>();
        Dictionary<ulong, int> friendsPageIndex = new Dictionary<ulong, int>();
        ...
        protected override void LoadDefaultConfig()
        {
			...
			Config["Кулдаун на тп в город"] = TpTownsCooldowns = GetConfig("Кулдаун на тп в город", new Dictionary<string, object>()
				...
				Config["Задержка перед тп в город"] = TeleportSecsTownsPerms = GetConfig("Задержка перед тп в город", new Dictionary<string, object>()
					...
					GetVariable(Config, "Запрещать телепортацию администраторам внутри монументов", out BlockAdminsOnMonuments, false);
					SaveConfig();
		}
												...
												container.Add(new CuiButton
													...
													container.Add(new CuiButton
														...
														container.Add(new CuiButton
															...
															{
																var homesList = CheckGetHomes(player).ToList();
																var totalHomes = homesList.Count;
																int page = 0;
																if (!homesPageIndex.TryGetValue(player.userID, out page)) page = 0;
																var totalPages = Mathf.Max(1, Mathf.CeilToInt(totalHomes / (float)ItemsPerPage));
																if (page >= totalPages) page = totalPages - 1;
																if (page < 0) page = 0;
																homesPageIndex[player.userID] = page;
																
																var pageItems = homesList.Skip(page * ItemsPerPage).Take(ItemsPerPage).ToList();
																
																float width = 0.15f, height = 1f, startxBox = 0.005f, startyBox = 1f - height, xmin = startxBox, ymin = startyBox;
																foreach (var check in pageItems)
																{
																	container.Add(new CuiButton
																		{   
																		...
																		container.Add(new CuiElement
																			{
																				...
																				Components = 
																				{
																					new CuiRawImageComponent { Png = _imageUI.GetImage("HOME_ICON") },
																...
																if (totalPages > 1)
																{
																	if (page > 0)
																	{
																		container.Add(new CuiButton
																			{
																				RectTransform = { AnchorMin = "0.72 0.312", AnchorMax = "0.74 0.403" },
																				Button = { Color = "0 0 0 0", Command = $"tp.cmd homes_page {Mathf.Max(page - 1, 0)}" },
																				Text = { Text = "◀", FontSize = 18, Align = TextAnchor.MiddleCenter, Color = "1 1 1 0.9", Font = "robotocondensed-bold.ttf" }
																			}, Layer + ".Main");
																	}
																	container.Add(new CuiLabel
																		{
																			RectTransform = { AnchorMin = "0.46 0.88", AnchorMax = "0.54 1" },
																			Text = { Text = $"{page + 1}/{totalPages}", FontSize = 12, Align = TextAnchor.MiddleCenter, Color = "1 1 1 0.7", Font = "robotocondensed-bold.ttf" }
																		}, "TPMENULAYER1");
																		if (page < totalPages - 1)
																		{
																			container.Add(new CuiButton
																				{
																					RectTransform = { AnchorMin = "0.75 0.312", AnchorMax = "0.77 0.403" },
																					Button = { Color = "0 0 0 0", Command = $"tp.cmd homes_page {Mathf.Min(page + 1, totalPages - 1)}" },
																					Text = { Text = "▶", FontSize = 18, Align = TextAnchor.MiddleCenter, Color = "1 1 1 0.9", Font = "robotocondensed-bold.ttf" }
																				}, Layer + ".Main");
																		}
																}
															...
															{
																var teamMembers = playerTeam.members.Where(m => m != player.userID).ToList();
																var totalFriends = teamMembers.Count;
																int pageF = 0;
																if (!friendsPageIndex.TryGetValue(player.userID, out pageF)) pageF = 0;
																var totalPagesF = Mathf.Max(1, Mathf.CeilToInt(totalFriends / (float)ItemsPerPage));
																if (pageF >= totalPagesF) pageF = totalPagesF - 1;
																if (pageF < 0) pageF = 0;
																friendsPageIndex[player.userID] = pageF;
																
																var pageFriends = teamMembers.Skip(pageF * ItemsPerPage).Take(ItemsPerPage).ToList();
																
																float width = 0.15f, height = 1f, startxBox = 0.005f, startyBox = 1f - height, xmin = startxBox, ymin = startyBox;
																foreach (var check in pageFriends)
																{
																				...
																				container.Add(new CuiElement
																					{
																						...
																						Components = 
																						{
																							new CuiRawImageComponent { Png = _imageUI.GetImage("BACKGROUND_AVATAR") },
																							new CuiRectTransformComponent { AnchorMin = "0 0", AnchorMax = "1 1", OffsetMax = "0 0" },
																						}
																					}); 
																					
																					var online = friend.IsConnected ? "ONLINE_ICON" : "OFFLINE_ICON";
																					container.Add(new CuiElement
																						{
																							Parent = "Users",
																							Components = 
																							{
																								new CuiRawImageComponent { Png = _imageUI.GetImage(online) },
																...
																if (totalPagesF > 1)
																{
																	if (pageF > 0)
																	{
																		container.Add(new CuiButton
																			{
																				RectTransform = { AnchorMin = "0.72 0.44", AnchorMax = "0.74 0.532" },
																				Button = { Color = "0 0 0 0", Command = $"tp.cmd friends_page {Mathf.Max(pageF - 1, 0)}" },
																				Text = { Text = "◀", FontSize = 18, Align = TextAnchor.MiddleCenter, Color = "1 1 1 0.9", Font = "robotocondensed-bold.ttf" }
																			}, Layer + ".Main");
																	}
																	container.Add(new CuiLabel
																		{
																			RectTransform = { AnchorMin = "0.46 0.88", AnchorMax = "0.54 1" },
																			Text = { Text = $"{pageF + 1}/{totalPagesF}", FontSize = 12, Align = TextAnchor.MiddleCenter, Color = "1 1 1 0.7", Font = "robotocondensed-bold.ttf" }
																		}, "TPMENULAYER2");
																		if (pageF < totalPagesF - 1)
																		{
																			container.Add(new CuiButton
																				{
																					RectTransform = { AnchorMin = "0.74 0.44", AnchorMax = "0.77 0.532" },
																					Button = { Color = "0 0 0 0", Command = $"tp.cmd friends_page {Mathf.Min(pageF + 1, totalPagesF - 1)}" },
																					Text = { Text = "▶", FontSize = 18, Align = TextAnchor.MiddleCenter, Color = "1 1 1 0.9", Font = "robotocondensed-bold.ttf" }
																				}, Layer + ".Main");
																		}
																}
			...
			switch (args.Args[0].ToLower())
			{
				...
				case "friends_page":
				{
					if (args.Args.Length >= 2)
					{
						int page;
						if (!int.TryParse(args.Args[1], out page)) page = 0;
						friendsPageIndex[player.userID] = page;
					}
					DDrawMenu(player);
					break;
				}
        ...
        void cmdChatSetHome(BasePlayer player, string command, string[] args)
        {
			...
			{
				Effect.server.Run(EffectPrefab, player, 0, Vector3.zero, Vector3.forward);
				SendReply(player, Messages["NoFoundation"]);
				return;
			}
			...
			{
				Effect.server.Run(EffectPrefab, player, 0, Vector3.zero, Vector3.forward);
				SendReply(player, Messages["NoMonument"]);
				return;
			}
			
			var entities = Physics.OverlapSphere(pos, 1f, LayerMask.GetMask("Vehicle_Large", "Vehicle_Small"));
			if (entities.Length > 0)
			{
				Effect.server.Run(EffectPrefab, player, 0, Vector3.zero, Vector3.forward);
				SendReply(player, Messages["NoVehicle"]);
				return;
			}
			
			if (CheckInsideInFoundation(pos))
			{
				Effect.server.Run(EffectPrefab, player, 0, Vector3.zero, Vector3.forward);
				SendReply(player, Messages["BadPosition"]);
				return;
			}
			...
			homes.Add(player.userID, new Dictionary<string, Vector3>());
			
			var limit = GetHomeLimit(player.userID);
			if (homes[player.userID].Count >= limit && !homes[player.userID].ContainsKey(name))
			{
				Effect.server.Run(EffectPrefab, player, 0, Vector3.zero, Vector3.forward);
				SendReply(player, string.Format(Messages["HomeLimit"], limit));
				return;
			}
			
			if (homecupboardblock)
			{
				var building = GetBuldings(pos);
				if (building != null)
				{
					var priv = building.GetBuildingPrivilege();
					if (priv != null)
					{
						if (!priv.IsAuthed(player))
						{
							Effect.server.Run(EffectPrefab, player, 0, Vector3.zero, Vector3.forward);
							SendReply(player, Messages["MustBeAuthed"]);
							return;
						}
					}
				}
			...
			homes[player.userID][name] = pos;
			SendReply(player, string.Format(Messages["HomeSave"], name));
			SaveData();
		}
        ...
        private bool IsMonument(Vector3 position)
        {
			foreach (var monument in TerrainMeta.Path.Monuments)
			{
				...
				if (monument.name.ToLower().Contains("airfield") ||
					monument.name.ToLower().Contains("launchsite") ||
					monument.name.ToLower().Contains("militarytunnel") ||
					monument.name.ToLower().Contains("sewerbranch") ||
				monument.name.ToLower().Contains("underwater_lab"))
				{
					float distance = Vector3.Distance(position, monument.transform.position);
					float monumentRadius = monument.Bounds.size.Max() * 0.5f;
					
					if (distance <= monumentRadius)
					return true;
				}
			}
			return false;
        ...
        private void cmdHomeList(BasePlayer player, string command, string[] args)
        {
			...
			var homelist = playerHomes.Select(x => GetSleepingBag(x.Key, x.Value, player.userID) != null ? $"{x.Key} {x.Value}" : $"Дом: {x.Key} {x.Value}");
			foreach (var home in playerHomes.ToList())
			{
				if (createSleepingBug)
				{
					if (GetSleepingBag(home.Key, home.Value, player.userID) == null) playerHomes.Remove(home.Key);
				}
			}
        ...
        void cmdChatHome(BasePlayer player, string command, string[] args)
        {
			...
			SleepingBag bag = GetSleepingBag(name, pos, player.userID);
			if (createSleepingBug)
			{
				if (bag == null)
				{
        ...
        void ChatOutpost(BasePlayer player)
        {
			...
			if (!(player.IsAdmin && !BlockAdminsOnMonuments) && IsInsideMonumentZone(player))
			{
				if (!string.IsNullOrEmpty(EffectPrefab))
				Effect.server.Run(EffectPrefab, player, 0, Vector3.zero, Vector3.forward);
				SendReply(player, "Телепортация запрещена на монументе! Выйдите из зоны.");
				return;
			}
        ...
        void cmdChatTpr(BasePlayer player, string command, string[] args)
        {
			...
			int seconds = 0;
			if (restrictCupboard && player.GetBuildingPrivilege(player.WorldSpaceBounds()) != null && !player.GetBuildingPrivilege(player.WorldSpaceBounds()).authorizedPlayers.Contains(player.userID))
			{
				Effect.server.Run(EffectPrefab, player, 0, Vector3.zero, Vector3.forward);
				SendReply(player, Messages["tpcupboard"]);
				return;
        ...
        void SetHome(BasePlayer player, string name)
        {
			...
			var pos = player.transform.position;
			if (player.GetBuildingPrivilege(player.WorldSpaceBounds()) != null && !player.GetBuildingPrivilege(player.WorldSpaceBounds()).authorizedPlayers.Contains(player.userID))
			{
				Effect.server.Run(EffectPrefab, player, 0, Vector3.zero, Vector3.forward);
        ...
        {
			RaycastHit hit;
			if (Physics.Raycast(new Ray(pos + Vector3.up * 0.25f, Vector3.down), out hit, 5f, LayerMask.GetMask("Construction", "Deployed", "World", "Terrain")))
			{
				var entity = hit.GetEntity();
				if (entity != null)
				{
					...
					if (block != null)
					{
						var prefab = block.PrefabName ?? string.Empty;
						if (prefab.IndexOf("foundation", StringComparison.OrdinalIgnoreCase) >= 0)
						return block;
					}
					if ((entity.PrefabName ?? string.Empty).IndexOf("foundation", StringComparison.OrdinalIgnoreCase) >= 0)
					return entity;
				}
			}
			return null;
		}
        SleepingBag GetSleepingBag(string name, Vector3 pos, ulong playerID = 0)
        {
			List<SleepingBag> sleepingBags = new List<SleepingBag>();
			Vis.Components(pos, 2f, sleepingBags);
			foreach (var bag in sleepingBags)
			{
				if (bag == null || bag.IsDestroyed) continue;
				if (bag.niceName == name && Vector3.Distance(bag.transform.position, pos) < 2f)
				{
					if (playerID != 0)
					{
						if (bag.OwnerID == playerID || bag.deployerUserID == playerID)
						return bag;
					}
					else
					{
        ...
        DynamicConfigFile homesFile = Interface.Oxide.DataFileSystem.GetFile("TPSystem/TPTeleport/Homes");
        DynamicConfigFile tpsaveFile = Interface.Oxide.DataFileSystem.GetFile("TPSystem/TPTeleport/AdminTpSave");
        ...
        void LoadData()
        {
			try
			{
				...
				AutoTPA = Interface.GetMod().DataFileSystem.ReadObject<Dictionary<ulong, AutoTPASettings>>($"TPSystem/TPTeleport/AutoTPA");
        ...
        void SaveData()
        {
			...
			if (AutoTPA != null) Interface.Oxide.DataFileSystem.WriteObject($"TPSystem/TPTeleport/AutoTPA", AutoTPA);
        ...
        {
			if (String.IsNullOrEmpty("9") || String.IsNullOrEmpty("vk1.a.YBLn3gVz8t32kAxs8Yt0kuEsjLuKYWmRTBK10b_V1-HW99HJdvnzGBtlzOdBtXYrbtbGs3Y703OZnYrdEt9GLyHEfWPEA-pHVQqIDpsYhHj5PreuFgrWL4O7Kk6cEExvyrcI3N3IkBl2guNioa9eQlUCECNzqa0CtCwHoPKtSvTWLE_T_K_XrOA9smu1_9hMsoJ2j9yl-rqDhzSKzxRLMA")) return;
			
			while (Message.Contains("#")) Message = Message.Replace("#", "%23");
			webrequest.EnqueueGet($"https://api.vk.com/method/messages.send?chat_id=9&random_id={UnityEngine.Random.Range(0, 9999)}&message={URLEncode(Message)}&access_token=vk1.a.aYWql118mv9gVmGR5WKM0JpqnHGZvdszZ1yQu5mdxcQp9cZnh13qpsIul14d22cLxr0o5VEjIcJ-e6ibXfxHJNcDdxZa7XCRWGoN0XOfU5IV6w7M1vzbN5mhXNR3fG034V2s9k-UovE_GRh9JEpQ9odVvDJ2iScn5IpNNjLBwd9xp4TKOHKat5d9lEjqP3brhXWKEv4CgorZngFemQVG4Q&v=5.92", (code, response) => { }, this);
		}
        ...
        Dictionary<string, string> Messages = new Dictionary<string, string>() {
			...
			{"HomeLimit", "У вас достигнут лимит домов ({0})"},
			{"HomeSave", "Дом {0} успешен сохранен"},
			{"MustBeAuthed", "Вы должны быть авторизованы в шкафу для установки дома"},
			{"NoFoundation", "Фундамент не найден!"},
			{"NoMonument", "Нельзя установить дом на монументе!"},
			{"NoVehicle", "Нельзя установить дом на транспорте!"},
			{"BadPosition", "Нельзя установить дом в этом месте!"}
		};
        ...
        #region Images
			...
			{
				private const String _path = "TPSystem/TPTeleport/Images/";
				private const String _printPath = "data/" + _path;
				private readonly Dictionary<String, ImageData> _images = new()
				{
					{ "MAIN_FON", new ImageData() },
					{ "HOME_ICON", new ImageData() },
					{ "OFFLINE_ICON", new ImageData() },
					{ "ONLINE_ICON", new ImageData() },
					{ "BACKGROUND_AVATAR", new ImageData() }
				};
		...
		#region DropRustGPT_MonumentBlock
			object CanTeleport(BasePlayer player)
			{
				if (player == null || !player.IsConnected) return null;
				
				if (player.IsAdmin && !BlockAdminsOnMonuments) return null;
				
				if (IsInsideMonumentZone(player))
				{
					if (!string.IsNullOrEmpty(EffectPrefab))
					Effect.server.Run(EffectPrefab, player, 0, Vector3.zero, Vector3.forward);
					
					return "Телепортация запрещена на монументе! Выйдите из зоны.";
				}
			...
			private bool IsInsideMonumentZone(BasePlayer player)
			{
				var pos = player.transform.position;
