...
{
	[Info("TPReportSystem", "pluginfuel.ru", "20.0.2")]
	class TPReportSystem : RustPlugin
	{
		#region Вар
			...
			private static TPReportSystem _;
		...
		#region Конфиг
			...
			public class Configuration
			{
				...
				[JsonProperty("Информация плагина")] public string Info = "{ Отличный плагин скилов всем советую, а кто не купит, тот гомосек";
				[JsonProperty("Оповещание")] public string Title = "Предоставте свой дискорд или Вк для проверки.\\nВведите команду /contact\\nЕсли Вы покинете сервер, Вы будете забанены на проекте AxionRust.\\nУ вас есть 5 минут!";
				[JsonProperty("Webhook")] public String WebhookNotify;
				[JsonProperty("Цвет сообщения в Discord (Можно найти на сайте - old.message.style/dashboard в разделе JSON)")] public Int32 Color;
        ...
        #region Хуки
			...
			void OnServerInitialized() {
				...
				if (Interface.Oxide.DataFileSystem.ExistsDatafile("TPSystem/TPReport/Player"))
                DB = Interface.Oxide.DataFileSystem.ReadObject<Dictionary<ulong, DataBase>>("TPSystem/TPReport/Player");
				else
			...
			void SaveDataBase() => Interface.Oxide.DataFileSystem.WriteObject("TPSystem/TPReport/Player", DB);
			...
			void ConsoleReport(ConsoleSystem.Arg args) {
				...
				if (player != null && args.HasArgs(1))
				{
					...
					if (args.Args[0] == "reason") {
						...
						if (target == null)
						{
							SendReply(player, "Игрок уже офлайн.");
							return;
						}
						...
						plugins.Find("RustApp")?.Call("RA_ReportSend", 
							player.userID.ToString(), 
							target.userID.ToString(), 
							config.Reasons[args.Args[2]], 
						$"Жалоба от {player.displayName} на {target.displayName}");
						
						AutoDemoRecord?.Call(
							"API_Report",
							player.UserIDString, player.displayName,
							target.UserIDString, target.displayName,
							config.Reasons[args.Args[2]]
						);
					...
					if (args.Args[0] == "check")
					{
						...
						if (db.Enable == false)
						{
							...
							plugins.Find("RustApp")?.Call("RA_ReportSend", 
								...
								target.userID.ToString(), 
								"Вызов на проверку", 
							$"Модератор {player.displayName} вызвал игрока {target.displayName} на проверку");
					...
					if (args.Args[0] == "ban")
					{
						...
						plugins.Find("RustApp")?.Call("RA_ReportSend", 
							...
							target.userID.ToString(), 
							$"Бан: {args.Args[2]}", 
						$"Модератор {player.displayName} забанил игрока {target.displayName} по причине: {args.Args[2]}");
        ...
        #region Интерфейс
			void ReportUI(BasePlayer player) {
				if (_imageUI == null) {
					PrintError("ImageUI не инициализирован! Попробуйте позже.");
					return;
				}
											...
											container.Add(new CuiElement
												{
													Parent = "Moder",
													Components = {
														new CuiRawImageComponent { Png = _imageUI.GetImage("BUTTON"), Color = "1 1 1 1" },
											...
											container.Add(new CuiElement
												{
													Parent = "Players",
													Components = {
														new CuiRawImageComponent { Png = _imageUI.GetImage("PLAYER_FON") },
														new CuiRectTransformComponent { AnchorMin = "0 0", AnchorMax = $"1 1", OffsetMax = "0 0" },
													}
												});
					...
					container.Add(new CuiElement
						{
							Parent = "Report",
							Components = {
								new CuiRawImageComponent { Png = _imageUI.GetImage("REPORT_PANEL"), Color = "1 1 1 1" },
								new CuiRectTransformComponent { AnchorMin = "0 0", AnchorMax = "1 1", OffsetMax = "0 0" },
							}
						});
						...
						container.Add(new CuiElement
							{
								Parent = LayerCheck,
								Components = {
									new CuiRawImageComponent { Png = _imageUI.GetImage("MODERATOR_PANEL_1"), Color = "1 1 1 1" },
									new CuiRectTransformComponent { AnchorMin = "0 0", AnchorMax = "1 1", OffsetMax = "0 0" },
								}
							});
						...
						container.Add(new CuiElement
							{
								Parent = LayerCheck,
								Components = {
									new CuiRawImageComponent { Png = _imageUI.GetImage("MODERATOR_PANEL_2"), Color = "1 1 1 1" },
									new CuiRectTransformComponent { AnchorMin = "0 0", AnchorMax = "1 1", OffsetMax = "0 0" },
								}
							});
        ...
        #region Дискорд
			List<Fields> DT_PlayerSendReport(BasePlayer Sender, UInt64 TargetID, String Reason)
			{
				List<Fields> fields = new List<Fields>
				{
					...
					new Fields("Steam64ID", $"[{Sender.userID}](\x68\x74\x74\x70\x73\x3A\x2F\x2F\x73\x74\x65\x61\x6D\x63\x6F\x6D\x6D\x75\x6E\x69\x74\x79\x2E\x63\x6F\x6D\x2F\x70\x72\x6F\x66\x69\x6C\x65\x73\x2F{Sender.userID})", true),
					new Fields("", "", false),
					...
					new Fields("Steam64ID", $"[{TargetID}](\x68\x74\x74\x70\x73\x3A\x2F\x2F\x73\x74\x65\x61\x6D\x63\x6F\x6D\x6D\x75\x6E\x69\x74\x79\x2E\x63\x6F\x6D\x2F\x70\x72\x6F\x66\x69\x6C\x65\x73\x2F{TargetID})", true),
        ...
        #region Images
			...
			{
				private const String _path = "TPSystem/TPReport/Images/";
				private const String _printPath = "data/" + _path;
				private readonly Dictionary<String, ImageData> _images = new()
				{
					{ "MAIN_FON", new ImageData() },
					{ "PLAYER_FON", new ImageData() },
					{ "REPORT_PANEL", new ImageData() },
					{ "BUTTON", new ImageData() },
					{ "MODERATOR_PANEL_1", new ImageData() },
					{ "MODERATOR_PANEL_2", new ImageData() }
				};
