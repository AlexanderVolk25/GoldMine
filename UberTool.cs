...
{
    [Info("UberTool", "pluginfuel.ru", "20.0.2")]
    ...
    internal class UberTool : RustPlugin
    {
        ...
        public class EPlanner : MonoBehaviour
        {
            ...
            private bool GetCurrentTool()
            {
                ...
                else if (heldItem != null && heldItem.ShortPrefabName.Contains("xmas.lights"))
                {
                    ...
                    else if (isLightDeployer)
                    {
                        ...
                        if (player.serverInput.WasJustPressed(BTN.FIRE_PRIMARY))
                        {
                            if (Physics.Raycast(player.eyes.HeadRay(), out RaycastHit raycastHit, 5f))
                            {
                                ...
                                if (heldItem.GetComponent<ChristmasLights>() == null)
                                {
                                    component = GameManager.server.CreateEntity(
                                        "assets/prefabs/deployable/christmas/christmas_lights/christmaslights.deployed.prefab",
                                        raycastHit.point,
                                        Quaternion.LookRotation(raycastHit.normal, player.eyes.HeadUp()), true
                                    ).GetComponent<ChristmasLights>();

                                    component.Spawn();
                                    heldItem.gameObject.AddComponent<ChristmasLights>();
                                }
                                else
                                {
                                    component = heldItem.GetComponent<ChristmasLights>();
                                }

                                component.AddPoint(raycastHit.point, raycastHit.normal, 0);
                                component.AddLengthUsed(1);
                                component.SendNetworkUpdate(BasePlayer.NetworkQueue.Update);
                            }
                        }
                        else if (player.serverInput.WasJustPressed(BTN.FIRE_SECONDARY))
        {
            player.ChatMessage("Finishing placement is no longer needed in this Rust version.");
        }
                    }else if (isWireTool)
                    {
                    ...
                    else if (!ctvTrgt)
                    {
                        ...
                        else if (serverInput.WasJustPressed(controlButtons[CmdType.HammerAuthInfo]) && !serverInput.WasDown(controlButtons[CmdType.HammerTransform]) && (Instance.enableHammerTCInfo || Instance.enableHammerCodelockInfo))
                        {
                            ...
                            if (Instance.enableHammerTCInfo && rayEntity && rayEntity is BuildingPrivlidge)
                            {
                                ...
                                IList list = (rayEntity as BuildingPrivlidge).authorizedPlayers.ToList();
                                for (int i = 0; i < list.Count; i++)
                                {
                                    PlayerNameID nameID = (PlayerNameID)list[i];
        ...
        private void LoadVariables()
        {
            ...
            varChatToggle = Convert.ToString(GetConfig("Commands", "Plugin toggle by chat", "12"));
