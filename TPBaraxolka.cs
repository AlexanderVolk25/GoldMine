...
{
	[Info("TPBaraxolka", "pluginfuel.ru", "20.0")]
	internal class TPBaraxolka : RustPlugin
	{
		...
		void OnServerInitialized()
		{
			...
			if (ImageLibrary)
			{
				...
				foreach (var check in config.bottleSetting.CustomItemsShop.Where(p => string.IsNullOrEmpty(p.ImageURL)))
					ImageLibrary.Call("AddImage", $"https://rustexplore.com/images/130/{check.defaultItem.ShortName}.png", check.defaultItem.ShortName + 128);
			}
		...
		private void DrawNPCUI(BasePlayer player, string img = "MAIN_FON", int a=0)
		{
			...
			container.Add(new CuiElement
			{
				...
				Components = 
				{
					new CuiRawImageComponent { Png = _imageUI.GetImage(img), Color = "1 1 1 1" },
		...
		void CreateInfoPlayer(BasePlayer player, bool opened)
		{
			...
			container.Add(new CuiElement
			{
				...
				Components =
						{
							new CuiRawImageComponent {FadeIn = 1f,Png = _imageUI.GetImage("ICON_PACKAGE"), Color = "0.929 0.882 0.847 1"},
		...
		private void InitializeLayers(BasePlayer player, string SelectMenu = "")
		{
			...
			container.Add(new CuiButton
			{
				...
				Button = { Close = "Menu_UI", Color = "1 1 1 0" },
			}, InitialLayer + ".C");
		...
		void CreateInfoJson(BasePlayer player, int page)
		{
			...
			DrawNPCUI(player, "BACKGROUND_DROP");
			...
			container.Add(new CuiPanel()
			{
				RectTransform = { AnchorMin = "1.04 -0.5", AnchorMax = "2.06 1" },
				Image = { Color = "1 1 1 0" }
			}, InitialLayer + ".Expedition", InitialLayer + ".ItemsList");

			var pos = GetPositions(7, 6, 0.01f, 0.02f);
			int count = 0;
			var itemsList = config.expeditionSettings.ItemsAdded.OrderBy(p => p.amount).Skip(page * 42).Take(42).ToList();
			foreach (var item in itemsList)
			{
				...
				}, InitialLayer + ".ItemsList", InitialLayer + item.skinid);

				
				container.Add(new CuiElement
				{
					Parent = InitialLayer + item.skinid,
					Components =
							{
								new CuiImageComponent { ItemId = ItemManager.FindItemDefinition(item.shortname).itemid, SkinId = item.skinid, Color = "1 1 1 0.6"},
				...
				container.Add(new CuiElement
				{
					...
					Components =
						{
							new CuiTextComponent { Color = "0.929 0.882 0.847 0.2", Text = $"<b>{item.amount}</b>", Align = TextAnchor.MiddleCenter , FontSize = 23,Font="robotocondensed-regular.ttf"},
							new CuiRectTransformComponent {AnchorMin = "0 0", AnchorMax = "1 1" }
							}		
	});		
	...
	Components =		
	{		
	new CuiTextComponent { Color = "0.929 0.882 0.847 0.3", Text = $"MIN {item.minCount}", Align = TextAnchor.LowerCenter , FontSize = 8,Font="robotocondensed-regular.ttf"},		
	new CuiRectTransformComponent {AnchorMin = "0 0.05", AnchorMax = "1 1", OffsetMin = "0 -3", OffsetMax="0 0" }
				...
				Components =
						{
							...
							new CuiRectTransformComponent {AnchorMin = "0 0", AnchorMax = "1 1.08" }
						}
			});
				...
				Components =
						{
							...
							new CuiRectTransformComponent {AnchorMin = "1 0", AnchorMax = "2 1.08" }
						}
			});
                ...
                Components =
                        {
                            ...
                            new CuiRectTransformComponent {AnchorMin = $"-0.52 -4.04", AnchorMax = $"0.6 -3.55"}
                        }
            });
            ...
            container.Add(new CuiButton
            {
                ...
                RectTransform =
                {
                    AnchorMin = $"-0.35 -3.54", AnchorMax = $"0.45 -3.05"
                }
		...
		void CreateInfo(BasePlayer player)
		{
			...
			container.Add(new CuiElement
			{
				...
				Components =
						{
							new CuiTextComponent { Color = "1 1 1 0.6", Text = "В чем польза Барахолки?", Align = TextAnchor.LowerCenter , FontSize = 20},
		...
		{
		Button = {
		Command = $"playerVideoTPBaraxolka {config.urlvido}",
		Color = "0 0 0 0.5" ,
		},
		Text = {
		...
		[ConsoleCommand("playerVideoTPBaraxolka")]
		...
		{
			DrawNPCUI(player, "BACKGROUND_BUY", 1);
			CuiElementContainer container = new CuiElementContainer();
			float amount = (float)TPEconomic?.Call("API_GET_BALANCE", (ulong) player.userID);
			...
			foreach (var item in itemList.Skip(page * 12).Take(12))
			{
				...
				container.Add(new CuiPanel()
				{
					RectTransform = { AnchorMin = $"{0+x} {0.77 - y}", AnchorMax = $"{0.32+x} {1 - y}" },
					Image = { Color = "1 1 1 0"}
				...
				container.Add(new CuiElement
				{
					...
					Components =
					{
						...
						new CuiRectTransformComponent {AnchorMin = "0 0.08", AnchorMax = "0.34 0.9"}
					}
				});
				...
				container.Add(new CuiLabel
				{
					...
					Text = { Text = $"{item.NeedGold}$", Align = TextAnchor.MiddleLeft, Font = "robotocondensed-regular.ttf", FontSize = 12, Color = "0.929 0.882 0.847 0.7" }
				}, InitialLayer + $".Shop.{i}");
				...
				container.Add(new CuiButton
				{
					Button = { Color = color, Command = $"UI_TPBaraxolka buy {i}", Material = "" },
		...
		public void DrawMenuPoints(BasePlayer player, MenuPoints choosed = null)
		{
			...
			container.Add(new CuiButton
				{
					RectTransform = { AnchorMin = "-1.5 0.655", AnchorMax = "-1.12 0.695", },
			...
			container.Add(new CuiButton
				{
					RectTransform = { AnchorMin = "-1.05 0.655", AnchorMax = "-0.85 0.695", },
			...
			container.Add(new CuiButton
				{
					RectTransform = { AnchorMin = "-0.78 0.655", AnchorMax = "-0.47 0.695", },
		...
		void cmdMenuTPBaraxolka(ConsoleSystem.Arg args)
		{
			...
			switch (args.Args[0])
			{
				...
				case "sendLoot":
					...
					List<ItemsAddSetting> items = new List<ItemsAddSetting>();

					foreach(var a in entityLoot.entitySource.GetComponent<ExpeditionExceptionBox>().storage.inventory.itemList)
					{
						int index = items.FindIndex(item1 => item1.shortname == a.info.shortname && item1.skinid == a.skin);
						if(index == -1)
						{
							ItemsAddSetting newItem = new ItemsAddSetting
							{
						...
						{
							items[index].amount += a.amount;
						}
					}
					foreach(var a in entityLoot.entitySource.GetComponent<ExpeditionExceptionBox>().storage.inventory.itemList)
					
					{
						int index = config.expeditionSettings.ItemsAdded.FindIndex(item1 => item1.shortname == a.info.shortname && item1.skinid == a.skin);
						int indexList = items.FindIndex(item1 => item1.shortname == a.info.shortname && item1.skinid == a.skin);
						if(index == -1) 
						{
							player.ChatMessage("Вы добавили предмет, который нельзя сдать!");
							return;
						}
						if(items[indexList].amount < config.expeditionSettings.ItemsAdded[index].minCount)
						{
							player.ChatMessage($"Вы не достигли минимума {a.info.displayName.english}!\n{a.amount}/{config.expeditionSettings.ItemsAdded[index].minCount}");
							return;
						}
					}
					  
					entityLoot.entitySource.GetComponent<ExpeditionExceptionBox>().Close();
					player.SendConsoleCommand("chat.say /tpbaraxolka");
					break;
				...
				case "buy":
					...
					if (buyItem.Command.Count > 0)
					{
						...
						TPEconomic.Call("API_PUT_BALANCE_MINUS", (ulong)player.userID, (float)buyItem.NeedGold);                        
		...
		internal class ItemsAddSetting
		{
			...
			[JsonProperty("SkinID")]
			public ulong skinid;
			[JsonProperty("Минимальное количество предмета (у предметов у которых нету стака, устанавливайте значение 1 - так как больше одного работать не будет)!")]		
       	public int minCount;
		}
		...
		public class ExpeditionSettings
		{
			...
			public List<ItemsAddSetting> ItemsAdded;
		...
		void ReadData()
		{
			DataObmen = Oxide.Core.Interface.Oxide.DataFileSystem.ReadObject<Dictionary<ulong, Dictionary<string, int>>>("TPSystem/TPBaraxolka/Obmen");
		}
		void WriteData() {
			Oxide.Core.Interface.Oxide.DataFileSystem.WriteObject("TPSystem/TPBaraxolka/Obmen", DataObmen);
		}


		void sendmoney(BasePlayer player, float money)
		{
			TPEconomic.Call("API_PUT_BALANCE_PLUS", (ulong)player.userID, money);
		}
		...
		class FatherComponent : FacepunchBehaviour
		{
			...
			{
				Interface.Oxide.DataFileSystem.WriteObject("TPSystem/TPBaraxolka/CurrentExpeditions", CurrentExpeditions);
				Interface.Oxide.DataFileSystem.WriteObject("TPSystem/TPBaraxolka/EndExpeditions", EndExpeditions);
				Interface.Oxide.DataFileSystem.WriteObject("TPSystem/TPBaraxolka/Artefacts", ins.ArtefactsList);
			...
			public void LoadData()
			{
				try
				{
					CurrentExpeditions = Interface.Oxide.DataFileSystem.ReadObject<Dictionary<ulong, ExpeditionPlayer>>("TPSystem/TPBaraxolka/CurrentExpeditions");
					EndExpeditions = Interface.Oxide.DataFileSystem.ReadObject<Dictionary<ulong, float>>("TPSystem/TPBaraxolka/EndExpeditions");
					ins.ArtefactsList = Interface.Oxide.DataFileSystem.ReadObject<List<ulong>>("TPSystem/TPBaraxolka/Artefacts");
			...
			void CreateInfoPlayer(BasePlayer player, double time)
			{
				...
				container.Add(new CuiElement
				{
					...
					Components =
						{
							new CuiRawImageComponent {Png = _imageUI.GetImage("ICON_PACKAGE"), Color = "0.929 0.882 0.847 1"},
							new CuiRectTransformComponent {AnchorMin = "1 1", AnchorMax = "1 1", OffsetMin = "-30 -60", OffsetMax = "50 20"}
						}
				});
		...
		void LoadSounds()
		{
			try
			{
				Sounds = Interface.GetMod().DataFileSystem.ReadObject<Dictionary<string, SoundData>>("TPSystem/TPBaraxolka/Sounds");
			}
			catch
			{
		...
		private void SaveSoundData() => Interface.Oxide.DataFileSystem.WriteObject("TPSystem/TPBaraxolka/Sounds", Sounds);
				...
				case "delete":
					{
						if (!Sounds.ContainsKey(args[1]))
						{
							SendReply(player, "Нету такой записи");
							return;
						}
		...
		public class ExpeditionExceptionBox : MonoBehaviour
		{
			...
			public float PoitsInvectoryCount()
			{
				...
				for (int i = 0; i < storage.inventory.itemList.Count; i++)
				{
					...
					int index = ins.config.expeditionSettings.ItemsAdded.FindIndex(item1 => item1.shortname == item.info.shortname && item1.skinid == item.skin);
					if (index == -1)
						return amount;
					var amm = ins.config.expeditionSettings.ItemsAdded[index];
					amount += amm.amount * item.amount;
				}
		...
		object CanMoveItem(Item item, PlayerInventory playerLoot, ItemContainerId targetContainer, int targetSlot, int amount)
		{
			...
			int index = config.expeditionSettings.ItemsAdded.FindIndex(item1 => item1.shortname == item.info.shortname && item1.skinid == item.skin);
		...
		{
			private const String _path = "TPSystem/TPBaraxolka/images/";
			private const String _printPath = "data/" + _path;
			private readonly Dictionary<String, ImageData> _images = new()
			{
				{ "MAIN_FON", new ImageData() },
				{ "BACKGROUND_DROP", new ImageData() },
				{ "BACKGROUND_BUY", new ImageData() },
				{ "ICON_PACKAGE", new ImageData() }
			};
			...
			public void DownloadImage()
			{
				...
				else
				{
					...
					if (failedImages.Count > 0)
					{
						...
						ins.PrintError($"Не удалось загрузить следующие изображения: {images}. Возможно, вы не загрузили их в папку '{_printPath}'.");
						Interface.Oxide.UnloadPlugin(ins.Name);
					}
					else
					{
						ins.Puts($"{_images.Count} изображений успешно загружено!");
					}
				}
			}
