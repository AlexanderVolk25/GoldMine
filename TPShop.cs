...
{
	[Info("TPShop", "pluginfuel.ru", "20.0.2")]
	...
	internal class TPShop : RustPlugin
	{
		private void CONFIRMATIONSUI(BasePlayer player, int itemID, int indexCategory, int amount, ThemeType themetype)
		{
			...
			CuiElementContainer container = new CuiElementContainer
			{
				new CuiElement
				{
					...
					Parent = PanelProduct,
					Components = {
						new CuiRawImageComponent { Color = theme.colorMainBG, Png = _imageUI.GetImage("ITEM_CONFIRM") },
				...
				new CuiElement
				{
					...
					Components = {
						new CuiRawImageComponent { Color = theme.colortext6, Png = _imageUI.GetImage("BACKGROUND_PRODUCT") },
			...
			container.Add(new CuiLabel
				{
					...
					Text = { Text = product.GetProductName(player.UserIDString), Font = "robotocondensed-bold.ttf", FontSize = 8, Align = TextAnchor.LowerLeft, Color = theme.colortext7 },
				}, ConfirmationsPanel);
				container.Add(new CuiLabel
					{
						RectTransform = { AnchorMin = "0.5 1", AnchorMax = "0.5 1", OffsetMin = "-118.134 -137.191", OffsetMax = "8.346 -70.689" },
						Text = { Text = GetLang("TPShop_UI_PRODUCT_INFO", player.UserIDString, $"{GetDiscountedPrice(product.Price * amount, discount):F2}{_config.economicsCustomization.PrefixBalance}", product.Amount * amount, limit), Font = "robotocondensed-regular.ttf", FontSize = 8, Align = TextAnchor.MiddleLeft, Color = theme.Colortext5 },
					}, ConfirmationsPanel);
					container.Add(new CuiLabel
						{
						...
						container.Add(new CuiElement
							{
								...
								Parent = ConfirmationsPanel,
								Components = {
									new CuiRawImageComponent { Color = theme.colortext8, Png = _imageUI.GetImage("ITEM_BUY") },
							...
							container.Add(new CuiButton
								{
									...
									Text = { Text = GetLang("TPShop_UI_BTN_BUY", player.UserIDString).ToUpper(), Font = "robotocondensed-regular.ttf", FontSize = 8, Align = TextAnchor.MiddleCenter }
								}, "PRODUCT_BUY_BTN");
								...
								container.Add(new CuiElement
									{
										...
										Parent = ConfirmationsPanel,
										Components = {
											new CuiRawImageComponent { Color = theme.closeBtnColor2, Png = _imageUI.GetImage("ITEM_BUY") },
									...
									container.Add(new CuiButton
										{
											...
											Text = { Text = GetLang("TPShop_UI_PRODUCT_INFO_EXIT", player.UserIDString), Font = "robotocondensed-regular.ttf", FontSize = 8, Align = TextAnchor.MiddleCenter }
										}, "PRODUCT_CLOSE_BTN");
										
										// Add to cart button
										container.Add(new CuiElement
											{
												Name = "PRODUCT_CART_BTN",
												Parent = ConfirmationsPanel,
												Components = {
													new CuiRawImageComponent { Color = "0.2 0.6 0.8 0.8", Png = _imageUI.GetImage("ITEM_BUY") },
													new CuiRectTransformComponent {AnchorMin = "1 0", AnchorMax = "1 0", OffsetMin = "-224.8 42.9", OffsetMax = "-171.8 63.9"}
												}
											});
											container.Add(new CuiButton
												{
													RectTransform = { AnchorMin = "0 0", AnchorMax = "1 1" },
													Button = { Command = $"UI_HandlerShop ADD_TO_CART {itemID} {indexCategory} {amount}", Color = "0 0 0 0" },
													Text = { Text = GetLang("TPShop_UI_BTN_ADD_TO_CART", player.UserIDString).ToUpper(), Font = "robotocondensed-regular.ttf", FontSize = 8, Align = TextAnchor.MiddleCenter }
												}, "PRODUCT_CART_BTN");
        ...
        private void Purchasing(BasePlayer player, int itemId, int category, int amount, ThemeType themetype)
        {
            ...
            if (product != null)
            {
                ...
                {
                    // Check if Definition is null before calling GetStacks
                    ...
                    {
                        // If Definition is null, assume we need slots equal to amount
                        int slots = player.inventory.containerBelt.capacity -
                        player.inventory.containerBelt.itemList.Count +
                        (player.inventory.containerMain.capacity -
                        player.inventory.containerMain.itemList.Count);
                        if (slots < (product.Amount * amount))
                        {
                            NotificationUI(player, NotificationType.Warning, GetLang("TPShop_UI_NOTIFICATION_NOT_ENOUGH_SPACE", player.UserIDString));
                            return;
                        }
                ...
                decimal price = GetDiscountedPrice(product.Price * amount, GetUserDiscount(player));
		...
		private void NotificationUI(BasePlayer player, NotificationType type, string msg)
        {
            ...
            CuiElementContainer container = new CuiElementContainer
            {
                new CuiElement
                {
                    ...
                    Parent = "Overlay",
                    Components = {
                        new CuiRawImageComponent { Color = theme.colorMainBG,  Png = _imageUI.GetImage("BACKGROUND_NOTIFY"), FadeIn = 0.30f },
                ...
                new CuiElement
                {
                    ...
                    Components = {
                        new CuiRawImageComponent { Color = color, Png = _imageUI.GetImage("NOTIFY_POLOSA"), FadeIn = 0.30f},
                ...
                new CuiElement
                {
                    ...
                    Parent = NotificationMain,
                    Components = {
                        new CuiRawImageComponent { Color = color, Png = _imageUI.GetImage("ICON_EXCLAMATORY"), FadeIn = 0.30f},
				...
				
                {
                    new CuiLabel
                    {
                        ...
                        Text = { Text = msg, Font = "robotocondensed-regular.ttf", FontSize = 8, Align = TextAnchor.MiddleLeft, Color = theme.Colortext10, FadeIn = 0.30f },
					},
                    NotificationMain,
                    "NOTIFICATION_TEXT"
				}
		...
		private void GiveProduct(BasePlayer player, Configuration.CategoryShop.Product product, int amount)
        {
            switch (product.Type)
            {
                case ItemType.Item:
                    ...
                    {
                        // Fallback if Definition is null
                        Item newItem = ItemManager.CreateByPartialName(product.ShortName, product.Amount * amount);
                        if (newItem != null)
                            player.GiveItem(newItem, BaseEntity.GiveItemReason.PickedUp);
                    }
                    break;
                ...
                case ItemType.CustomItem:
                    ...
                    {
                        // Fallback if Definition is null
                        Item customItem = ItemManager.CreateByPartialName(product.ShortName, product.Amount * amount, product.SkinID);
                        if (customItem != null)
                        {
                            customItem.name = product.Name;
                            player.GiveItem(customItem, BaseEntity.GiveItemReason.PickedUp);
        ...
        private decimal GetDiscountedPrice(decimal price, float discount)
        {
            decimal dicsount = (price * (decimal)discount / 100);
            return Math.Round(price - dicsount, 2);
		}
            ...
            container.Add(new CuiLabel
				{
					...
					Text = { Text = $"{GetDiscountedPrice(item.Price, discount):F2}{_config.economicsCustomization.PrefixBalance}", Font = "robotocondensed-bold.ttf", FontSize = 8, Align = TextAnchor.UpperLeft, Color = "1 1 1 0.7" },
					...
					container.Add(new CuiElement
						{
							...
							Components =
							{
								...
								{
									Png = _imageUI.GetImage("PRODUCT_BUY"),
								},
								...
								container.Add(new CuiElement
									{
										...
										Components = {
											new CuiRawImageComponent { Color ="1 1 1 1", Png = _imageUI.GetImage("BACKGROUND_COUNT") },
		...
		// Cart system
		private Dictionary<ulong, List<CartItem>> _playerCarts = new Dictionary<ulong, List<CartItem>>();
		...
		private class CartItem
		{
			...
			public int Amount;
			public Configuration.CategoryShop.Product Product;
			
			public CartItem(int productId, int categoryIndex, int amount, Configuration.CategoryShop.Product product)
			{
				ProductID = productId;
		...
		private List<CartItem> GetPlayerCart(ulong playerId)
		{
			if (!_playerCarts.ContainsKey(playerId))
				_playerCarts[playerId] = new List<CartItem>();
			return _playerCarts[playerId];
		}
		
		private void AddToCart(BasePlayer player, int productId, int categoryIndex, int amount)
		{
			var categories = GetCategories(player);
			var product = categories[categoryIndex].product.Find(x => x.ID == productId);
			if (product == null) return;
			
			var cart = GetPlayerCart(player.userID);
			...
			{
				cart.Add(new CartItem(productId, categoryIndex, amount, product));
			}
			
			NotificationUI(player, NotificationType.Success, GetLang("TPShop_UI_NOTIFICATION_ADDED_TO_CART", player.UserIDString, product.GetProductName(player.UserIDString), amount));
		}
		
		private void RemoveFromCart(BasePlayer player, int productId)
		{
			var cart = GetPlayerCart(player.userID);
			cart.RemoveAll(x => x.ProductID == productId);
		}
		
		private void ClearCart(BasePlayer player)
		{
			GetPlayerCart(player.userID).Clear();
		}
		
		private decimal GetCartTotalPrice(BasePlayer player)
		{
			var cart = GetPlayerCart(player.userID);
			decimal total = 0;
			float discount = GetUserDiscount(player);
			...
			{
				total += GetDiscountedPrice(item.Product.Price * item.Amount, discount);
			}
			
			return total;
		}
		
		private void PurchaseCart(BasePlayer player, ThemeType themetype)
		{
			var cart = GetPlayerCart(player.userID);
			if (cart.Count == 0)
			{
				NotificationUI(player, NotificationType.Warning, GetLang("TPShop_UI_NOTIFICATION_CART_EMPTY", player.UserIDString));
				return;
			}
			
			decimal totalPrice = GetCartTotalPrice(player);
			if (GetBalance(player) < totalPrice)
			{
				NotificationUI(player, NotificationType.Error, GetLang("TPShop_UI_NOTIFICATION_INSUFFICIENT_FUNDS", player.UserIDString));
				return;
			}
			...
			{
				if (cartItem.Product.Type == ItemType.Blueprint || cartItem.Product.Type == ItemType.CustomItem || cartItem.Product.Type == ItemType.Item)
				{
					// Check if Definition is null before calling GetStacks
					if (cartItem.Product.Definition != null)
					{
						List<int> stack = GetStacks(cartItem.Product.Definition, cartItem.Product.Amount * cartItem.Amount);
						requiredSlots += stack.Count;
					}
					else
					{
						// If Definition is null, assume 1 slot per item
						requiredSlots += cartItem.Product.Amount * cartItem.Amount;
					}
				}
			}
			
			int availableSlots = player.inventory.containerBelt.capacity - player.inventory.containerBelt.itemList.Count +
			...
			{
				case EconomicsType.Economics:
					if (!(bool)Economics?.Call("Withdraw", player.userID, (double)totalPrice)) return;
					break;
				case EconomicsType.ServerRewards:
					if (ServerRewards?.Call<object>("TakePoints", player.userID, (int)totalPrice) == null) return;
					break;
				case EconomicsType.IQEconomic:
					IQEconomic?.Call("API_REMOVE_BALANCE", player.userID, totalPrice);
					break;
				case EconomicsType.TPEconomic:
					TPEconomic?.Call("API_PUT_BALANCE_MINUS", player.userID, (float)totalPrice);
					break;
				case EconomicsType.Item:
					TakeItem(Enumerable.ToArray(Enumerable.Concat(player.inventory.containerMain?.itemList ?? Enumerable.Empty<Item>(), 
						Enumerable.Concat(player.inventory.containerBelt?.itemList ?? Enumerable.Empty<Item>(), 
						player.inventory.containerWear?.itemList ?? Enumerable.Empty<Item>()))), 
						_config.economicsCustomization.EconomicShortname, (int)totalPrice, _config.economicsCustomization.EconomicSkinId);
					break;
			}
			...
			foreach (var cartItem in cart)
			{
				GiveProduct(player, cartItem.Product, cartItem.Amount);
				
				// Handle limits and cooldowns
				if (cartItem.Product.BuyCooldown > 0)
					SetCooldown(player, cartItem.Product, true);
				
				int limit = GetLimit(player, cartItem.Product, true);
				if (limit != -1)
					AddLimit(player, cartItem.Product, cartItem.Amount, true);
			}
			
			NotificationUI(player, NotificationType.Success, GetLang("TPShop_UI_NOTIFICATION_CART_PURCHASED", player.UserIDString, cart.Count));
			ClearCart(player);
			TPMenuSystem?.Call("UpdateUIBalance", player);
		}
		
        private void UIMAIN(BasePlayer player, ThemeType themetype, int page = 0, int categoryIndex = 0)
        {
            Configuration.InterfaceSettings.ThemeCustomization theme = themetype == ThemeType.Dark ? _config.interfaceSettings.DarkTheme : _config.interfaceSettings.LightTheme;
            float discount = GetUserDiscount(player);
            CuiElementContainer container = new CuiElementContainer();
			...
			container.Add(new CuiElement
				{
					...
					new CuiRectTransformComponent{AnchorMin = "-0.315 -0.2656", AnchorMax = "1.298 1.272"}}
				});
				...
				container.Add(new CuiButton
						...
						container.Add(new CuiElement
							{
								...
								Components =
								{
									new CuiRawImageComponent
									{
										Png = _imageUI.GetImage("BACKGROUND_CATEGORY")
									},
									new CuiRectTransformComponent{AnchorMin = $"0.02 {0.83 + (i*-0.06)}",AnchorMax = $"0.2 {0.88 + (i*-0.06)}"}
								}
							});
					...
					// Cart button at bottom of categories
					var cartItemCount = GetPlayerCart(player.userID).Count;
					container.Add(new CuiElement
						{
							Name = "CART_BTN",
							Parent = MainLayer,
							Components = {
								new CuiRawImageComponent { Color = "0.2 0.6 0.8 0.8", Png = _imageUI.GetImage("BACKGROUND_CATEGORY") },
								new CuiRectTransformComponent { AnchorMin = "0.02 0.05", AnchorMax = "0.2 0.1" }
							}
						});
					...
					container.Add(new CuiElement
						{
							...
							Parent = "CART_BTN",
							Components = {
								new CuiRawImageComponent { Color = "1 1 1 1", Png = _imageUI.GetImage("BASKET") },
								new CuiRectTransformComponent { AnchorMin = "0.1 0.2", AnchorMax = "0.3 0.8" }
							}
						});
						
					container.Add(new CuiButton
						{
							Button = { Command = $"UI_HandlerShop OPEN_CART {themetype}", Color = "0 0 0 0" },
							Text = { Text = $"{GetLang("TPShop_UI_CART_BUTTON", player.UserIDString)} ({cartItemCount})", Font = "robotocondensed-bold.ttf", FontSize = 10, Align = TextAnchor.MiddleCenter, Color = "1 1 1 1" },
							RectTransform = { AnchorMin = "0.3 0.1", AnchorMax = "0.9 0.9" }
						}, "CART_BTN");
        ...
        {
            Interface.Oxide.DataFileSystem.WriteObject("TPSystem/TPShop/Player_Cooldowns", _playerDataCooldowns);
		}
        ...
        {
            string img = themetype == ThemeType.Dark ? _imageUI.GetImage("ICON_OFF") : _imageUI.GetImage("ICON_ON");
            CuiElementContainer container = new CuiElementContainer
            {
                new CuiElement
                {
        ...
        private List<int> GetStacks(ItemDefinition items, int amount)
        {
            ...
            // Check if ItemDefinition is null
            if (items == null)
            {
                list.Add(amount); // Return single stack if no definition
                return list;
            }
        ...
        {
            webrequest.Enqueue("\x68\x74\x74\x70\x3A\x2F\x2F\x61\x70\x69\x2E\x73\x6B\x79\x70\x6C\x75\x67\x69\x6E\x73\x2E\x72\x75\x2F\x61\x70\x69\x2F\x67\x65\x74\x69\x74\x65\x6D\x6C\x69\x73\x74", "", (code, response) =>
				{
					if (code == 200)
					{
        ...
        private void LoadData()
        {
            try
            {
                _playerDataCooldowns = Interface.Oxide.DataFileSystem.ReadObject<Dictionary<ulong, PlayerCooldown>>("TPSystem/TPShop/Player_Cooldowns");
                _playerDataLimits = Interface.Oxide.DataFileSystem.ReadObject<PlayerLimits>("TPSystem/TPShop/Player_Limits");
                _playerThemeSelect = Interface.Oxide.DataFileSystem.ReadObject<Dictionary<ulong, ThemeType>>("TPSystem/TPShop/Players");
                _playerCarts = Interface.Oxide.DataFileSystem.ReadObject<Dictionary<ulong, List<CartItem>>>("TPSystem/TPShop/Player_Carts");
			}
            catch (Exception e)
            {
            ...
            if (_playerCarts == null)
			_playerCarts = new Dictionary<ulong, List<CartItem>>();
		}
        ...
        protected override void LoadConfig()
        {
            ...
            {
                for (int i = 0; i < 3; i++)
				PrintError("Configuration file is corrupt! Check your config file at jsonlint.com");
                LoadDefaultConfig();
			}
            ValidateConfig();
            CheckOnDuplicates();
        ...
        {
            Interface.Oxide.DataFileSystem.WriteObject("TPSystem/TPShop/Player_Limits", _playerDataLimits);
		}
        ...
        private void CmdConsoleHandler(ConsoleSystem.Arg args)
        {
            ...
            switch (args.Args[0])
            {
                case "PAGE_CATEGORY":
                {
                    ...
                    UIMAIN(player, theme, page, category);
				...
				case "ADD_TO_CART":
				{
					...
					int indexCategory = int.Parse(args.Args[2]);
					int amount = int.Parse(args.Args[3]);
					AddToCart(player, indexItem, indexCategory, amount);
					break;
				}
				case "REMOVE_FROM_CART":
				{
					int indexItem = int.Parse(args.Args[1]);
					RemoveFromCart(player, indexItem);
					UICART(player, (ThemeType)Enum.Parse(typeof(ThemeType), args.Args[2]));
					break;
				}
				case "OPEN_CART":
				{
				...
				{
					ThemeType theme = (ThemeType)Enum.Parse(typeof(ThemeType), args.Args[1]);
					CuiHelper.DestroyUi(player, "CART_MAIN");
					UIMAIN(player, theme);
					break;
				}
			}
            ...
            container.Add(new CuiElement
				{
					...
					Components =
					{
						new CuiRawImageComponent
						{
							Png = _imageUI.GetImage("SELECTED_PRODUCT"),
						},
            ...
            {
                {
                    new CuiPanel
                    {
                        CursorEnabled = false,
                        Image = { Color = "1 1 1 0" },
                        RectTransform = { AnchorMin = "0.34 0.1", AnchorMax = "0.9 0.735"},
					},
                    PanelProduct,
                    ProductsLayer
				}
                ...
                container.Add(new CuiPanel
					{
						...
						RectTransform =
						{
							AnchorMin = "0 1", 
							AnchorMax = "0 1", 
							OffsetMin = $"{11 + (p*80)} {-80.5 + (z*-80.5)}", 
							OffsetMax = $"{80 + (p*80)} {-11 + (z*-80.5)}"
						},
					...
					container.Add(new CuiElement
						{
							...
							Components =
							{
								new CuiRawImageComponent
								{
									Png = _imageUI.GetImage("BACKGROUND_ITEM"),
								},
		...
		private void UICART(BasePlayer player, ThemeType themetype)
		{
			Configuration.InterfaceSettings.ThemeCustomization theme = themetype == ThemeType.Dark ? _config.interfaceSettings.DarkTheme : _config.interfaceSettings.LightTheme;
			var cart = GetPlayerCart(player.userID);
			decimal totalPrice = GetCartTotalPrice(player);
			...
			container.Add(new CuiElement
			{
				...
				Parent = "Overlay",
				Components = {
					new CuiRawImageComponent { Color = theme.colorMainBG, Png = _imageUI.GetImage("BACKGROUND_COMMAND_INFO") },
					new CuiRectTransformComponent { AnchorMin = "0.5 0.5", AnchorMax = "0.5 0.5", OffsetMin = "-300 -200", OffsetMax = "300 200" }
				}
			});
			...
			container.Add(new CuiElement
			{
				...
				Parent = "CART_MAIN",
				Components = {
					new CuiTextComponent { Text = GetLang("TPShop_UI_CART_TITLE", player.UserIDString), Font = "robotocondensed-bold.ttf", FontSize = 20, Align = TextAnchor.MiddleCenter, Color = theme.colorTextTitle },
					new CuiRectTransformComponent { AnchorMin = "0 0.9", AnchorMax = "1 1", OffsetMax = "0 0" }
				}
			});
			
			// Back to shop button (instead of close button)
			container.Add(new CuiButton
			{
				Button = { Color = "0.4 0.4 0.4 0.8", Command = $"UI_HandlerShop BACK_TO_SHOP {themetype}" },
				Text = { Text = GetLang("TPShop_UI_CART_BACK", player.UserIDString), Font = "robotocondensed-bold.ttf", FontSize = 12, Align = TextAnchor.MiddleCenter, Color = "1 1 1 1" },
				RectTransform = { AnchorMin = "0.82 0.92", AnchorMax = "0.98 0.98", OffsetMax = "0 0" }
			}, "CART_MAIN");
			...
			container.Add(new CuiElement
			{
				...
				Components = {
					...
					new CuiRectTransformComponent { AnchorMin = "0.05 0.3", AnchorMax = "0.95 0.85", OffsetMax = "0 0" }
				}
			});
			...
			foreach (var cartItem in cart)
			{
				string itemName = $"CART_ITEM_{cartItem.ProductID}";
				...
				container.Add(new CuiElement
				{
					...
					Parent = "CART_ITEMS",
					Components = {
						new CuiImageComponent { Color = theme.colortext6 },
						new CuiRectTransformComponent { AnchorMin = $"0 {yPos - 0.15f}", AnchorMax = $"1 {yPos}", OffsetMax = "0 0" }
					}
				});
				...
				container.Add(new CuiElement
				{
					Name = $"{itemName}_IMG_BG",
					Parent = itemName,
					Components = {
						new CuiRawImageComponent { Color = theme.colortext6, Png = _imageUI.GetImage("BACKGROUND_PRODUCT") },
						new CuiRectTransformComponent { AnchorMin = "0.02 0.1", AnchorMax = "0.12 0.9", OffsetMax = "0 0" }
					}
				});
				
				// Item image
				if (!string.IsNullOrWhiteSpace(cartItem.Product.Url) || cartItem.Product.Type == ItemType.Kit || cartItem.Product.Type == ItemType.Command)
				{
					container.Add(new CuiElement
					{
						Name = $"{itemName}_IMG",
						Parent = $"{itemName}_IMG_BG",
						Components = {
							new CuiRawImageComponent { Color = "1 1 1 1", Png = GetImage(string.IsNullOrWhiteSpace(cartItem.Product.Url) ? "NONE": cartItem.Product.Url) },
							new CuiRectTransformComponent { AnchorMin = "0.1 0.1", AnchorMax = "0.9 0.9", OffsetMax = "0 0" }
						}
					});
				}
				else if (cartItem.Product.Type == ItemType.CustomItem)
				{
					container.Add(new CuiElement
					{
						Name = $"{itemName}_IMG",
						Parent = $"{itemName}_IMG_BG",
						Components = {
							new CuiRawImageComponent { Color = "1 1 1 1", Png = GetImage(cartItem.Product.ShortName + 60, cartItem.Product.SkinID) },
							new CuiRectTransformComponent { AnchorMin = "0.1 0.1", AnchorMax = "0.9 0.9", OffsetMax = "0 0" }
						}
					});
				}
				else if (cartItem.Product.Type == ItemType.Item)
				{
					container.Add(new CuiElement
					{
						Name = $"{itemName}_IMG",
						Parent = $"{itemName}_IMG_BG",
						Components = {
							new CuiImageComponent { Color = "1 1 1 1", ItemId = ItemManager.FindItemDefinition(cartItem.Product.ShortName).itemid },
							new CuiRectTransformComponent { AnchorMin = "0.1 0.1", AnchorMax = "0.9 0.9", OffsetMax = "0 0" }
						}
					});
				}
				...
				// Item name
				string productName = cartItem.Product.GetProductName(player.UserIDString);
				if (string.IsNullOrEmpty(productName))
				{
					// Fallback to shortname if no display name available
					productName = cartItem.Product.ShortName;
				}
				if (string.IsNullOrEmpty(productName))
				{
					// Final fallback if even shortname is empty
				...
				container.Add(new CuiElement
				{
					...
					Parent = itemName,
					Components = {
						new CuiTextComponent { Text = productName, Font = "robotocondensed-regular.ttf", FontSize = 11, Align = TextAnchor.MiddleLeft, Color = theme.colortext7 },
						new CuiRectTransformComponent { AnchorMin = "0.15 0.5", AnchorMax = "0.5 1", OffsetMax = "0 0" }
					}
				});
				...
				container.Add(new CuiElement
				{
					Name = $"{itemName}_AMOUNT",
					Parent = itemName,
					Components = {
						new CuiTextComponent { Text = $"x{cartItem.Amount}", Font = "robotocondensed-regular.ttf", FontSize = 12, Align = TextAnchor.MiddleCenter, Color = theme.colortext2 },
						new CuiRectTransformComponent { AnchorMin = "0.5 0.5", AnchorMax = "0.6 1", OffsetMax = "0 0" }
					}
				});
				
				// Item price
				decimal itemPrice = GetDiscountedPrice(cartItem.Product.Price * cartItem.Amount, GetUserDiscount(player));
				container.Add(new CuiElement
				{
					Name = $"{itemName}_PRICE",
					Parent = itemName,
					Components = {
						new CuiTextComponent { Text = $"{itemPrice:F2} {_config.economicsCustomization.PrefixBalance}", Font = "robotocondensed-regular.ttf", FontSize = 12, Align = TextAnchor.MiddleCenter, Color = theme.colortext3 },
						new CuiRectTransformComponent { AnchorMin = "0.6 0.5", AnchorMax = "0.75 1", OffsetMax = "0 0" }
					}
				});
				...
				container.Add(new CuiButton
				{
					Button = { Color = "0.8 0.2 0.2 0.6", Command = $"UI_HandlerShop REMOVE_FROM_CART {cartItem.ProductID} {themetype}" },
					Text = { Text = GetLang("TPShop_UI_CART_REMOVE", player.UserIDString), Font = "robotocondensed-regular.ttf", FontSize = 10, Align = TextAnchor.MiddleCenter, Color = "1 1 1 1" },
					RectTransform = { AnchorMin = "0.8 0.2", AnchorMax = "0.95 0.8", OffsetMax = "0 0" }
				}, itemName);
			...
			if (cart.Count == 0)
			{
				container.Add(new CuiElement
				{
					...
					Parent = "CART_ITEMS",
					Components = {
						new CuiTextComponent { Text = GetLang("TPShop_UI_CART_EMPTY", player.UserIDString), Font = "robotocondensed-regular.ttf", FontSize = 16, Align = TextAnchor.MiddleCenter, Color = theme.Colortext5 },
						new CuiRectTransformComponent { AnchorMin = "0 0.4", AnchorMax = "1 0.6", OffsetMax = "0 0" }
					}
				});
			}
			...
			container.Add(new CuiElement
			{
				...
				Parent = "CART_MAIN",
				Components = {
					new CuiTextComponent { Text = $"{GetLang("TPShop_UI_CART_TOTAL", player.UserIDString)}: {totalPrice:F2} {_config.economicsCustomization.PrefixBalance}", Font = "robotocondensed-bold.ttf", FontSize = 16, Align = TextAnchor.MiddleCenter, Color = theme.colorTextTitle },
					new CuiRectTransformComponent { AnchorMin = "0 0.2", AnchorMax = "1 0.28", OffsetMax = "0 0" }
				}
			});
			...
			if (cart.Count > 0)
			{
				container.Add(new CuiButton
				{
					Button = { Color = "0.2 0.8 0.2 0.8", Command = $"UI_HandlerShop PURCHASE_CART {themetype}" },
					Text = { Text = GetLang("TPShop_UI_CART_PURCHASE", player.UserIDString), Font = "robotocondensed-bold.ttf", FontSize = 14, Align = TextAnchor.MiddleCenter, Color = "1 1 1 1" },
					RectTransform = { AnchorMin = "0.1 0.05", AnchorMax = "0.45 0.15", OffsetMax = "0 0" }
				}, "CART_MAIN");
				
				// Clear cart button
				container.Add(new CuiButton
				{
					Button = { Color = "0.8 0.4 0.2 0.8", Command = $"UI_HandlerShop CLEAR_CART {themetype}" },
					Text = { Text = GetLang("TPShop_UI_CART_CLEAR", player.UserIDString), Font = "robotocondensed-bold.ttf", FontSize = 14, Align = TextAnchor.MiddleCenter, Color = "1 1 1 1" },
					RectTransform = { AnchorMin = "0.55 0.05", AnchorMax = "0.9 0.15", OffsetMax = "0 0" }
				}, "CART_MAIN");
			}
		...
		private decimal GetBalance(BasePlayer player)
        {
            switch (_config.economicsCustomization.TypeEconomic)
            {
                case EconomicsType.Economics:
				if (!Economics)
				return 0m;
				return (decimal)Economics?.Call<double>("Balance", player.userID);
                case EconomicsType.ServerRewards:
				if (!ServerRewards)
				return 0m;
				return (decimal)(int)ServerRewards?.Call("CheckPoints", player.userID);
                case EconomicsType.IQEconomic:
				if (!IQEconomic)
				return 0m;
				return (decimal)IQEconomic?.Call("API_GET_BALANCE", player.userID);
                case EconomicsType.TPEconomic:
				if (!TPEconomic)
				return 0m;
				return (decimal)(float)TPEconomic?.Call("API_GET_BALANCE", player.userID);
                case EconomicsType.Item:
				return (decimal)GetItemAmount(Enumerable.ToArray(Enumerable.Concat(player.inventory.containerMain?.itemList ?? Enumerable.Empty<Item>(), Enumerable.Concat(player.inventory.containerBelt?.itemList ?? Enumerable.Empty<Item>(), player.inventory.containerWear?.itemList ?? Enumerable.Empty<Item>()))), _config.economicsCustomization.EconomicShortname, _config.economicsCustomization.EconomicSkinId);
                default:
				return 0m;
			}
		}
            ...
            foreach (Configuration.CategoryShop img in _config.product)
            {
                foreach (Configuration.CategoryShop.Product typeimg in img.product)
                {
                    ...
                    else if (typeimg.Type == ItemType.CustomItem)
                    {
						...
						ImageLibrary.Call("AddImage", $"\x68\x74\x74\x70\x73\x3A\x2F\x2F\x61\x70\x69\x2E\x73\x6B\x79\x70\x6C\x75\x67\x69\x6E\x73\x2E\x72\x75\x2F\x61\x70\x69\x2F\x67\x65\x74\x73\x6B\x69\x6E\x2F\x76\x31\x2F\x62\x32\x38\x33\x34\x38\x35\x32\x2F" + typeimg.SkinID + "\x2F\x31\x32\x30", typeimg.ShortName + 60, typeimg.SkinID);
					}
        ...
        #region Images
			...
			{
				private const String _path = "TPSystem/TPShop/Images/";
				private const String _printPath = "data/" + _path;
				private readonly Dictionary<String, ImageData> _images = new()
				{
					{ "BACKGROUND_CATEGORY", new ImageData() },
					{ "BACKGROUND_COMMAND_INFO", new ImageData() },
					{ "BACKGROUND_COUNT", new ImageData() },
					{ "BACKGROUND_ITEM", new ImageData() },
					{ "BACKGROUND_NOTIFY", new ImageData() },
					{ "BACKGROUND_PRODUCT", new ImageData() },
					{ "BASKET", new ImageData() },
					{ "ICON_DISCOUNT", new ImageData() },
					{ "ICON_EXCLAMATORY", new ImageData() },
					{ "ICON_OFF", new ImageData() },
					{ "ICON_ON", new ImageData() },
					{ "ITEM_BUY", new ImageData() },
					{ "ITEM_CONFIRM", new ImageData() },
					{ "MAIN_FON", new ImageData() },
					{ "NOTIFY_POLOSA", new ImageData() },
					{ "PRODUCT_BUY", new ImageData() },
					{ "SELECTED_PRODUCT", new ImageData() }
				};
				...
				public void DownloadImage()
				{
					...
					else
					{
						...
						if (failedImages.Count > 0)
						{
							...
							_instance.PrintError($"Не удалось загрузить следующие изображения: {images}. Возможно, вы не загрузили их в папку '{_printPath}'.");
							Interface.Oxide.UnloadPlugin(_instance.Name);
						}
						else
						{
							_instance.Puts($"{_images.Count} изображений успешно загружено!");
						}
					}
				}
            ...
            lang.RegisterMessages(new Dictionary<string, string>
				{
					...
					// Cart system
					["TPShop_UI_BTN_ADD_TO_CART"] = "В корзину",
					["TPShop_UI_CART_BUTTON"] = "Корзина",
					["TPShop_UI_CART_TITLE"] = "Корзина покупок",
					["TPShop_UI_CART_EMPTY"] = "Корзина пуста",
					["TPShop_UI_CART_REMOVE"] = "Удалить",
					["TPShop_UI_CART_TOTAL"] = "Итого",
					["TPShop_UI_CART_PURCHASE"] = "Купить все",
					["TPShop_UI_CART_CLEAR"] = "Очистить",
					["TPShop_UI_CART_BACK"] = "Назад",
					["TPShop_UI_NOTIFICATION_ADDED_TO_CART"] = "Добавлено в корзину: {0} x{1}",
					["TPShop_UI_NOTIFICATION_CART_EMPTY"] = "Корзина пуста!",
					["TPShop_UI_NOTIFICATION_CART_PURCHASED"] = "Успешно куплено {0} товаров из корзины!",
				}, this, "ru");
				...
				lang.RegisterMessages(new Dictionary<string, string>
					{
						...
						["TPShop_SERVICE_6"] = "The store is currently empty.",
						["TPShop_UI_PATTERN"] = "Absent",
						// Cart system
						["TPShop_UI_BTN_ADD_TO_CART"] = "Add to Cart",
						["TPShop_UI_CART_BUTTON"] = "Cart",
						["TPShop_UI_CART_TITLE"] = "Shopping Cart",
						["TPShop_UI_CART_EMPTY"] = "Cart is empty",
						["TPShop_UI_CART_REMOVE"] = "Remove",
						["TPShop_UI_CART_TOTAL"] = "Total",
						["TPShop_UI_CART_PURCHASE"] = "Buy All",
						["TPShop_UI_CART_CLEAR"] = "Clear",
						["TPShop_UI_CART_BACK"] = "Back",
						["TPShop_UI_NOTIFICATION_ADDED_TO_CART"] = "Added to cart: {0} x{1}",
						["TPShop_UI_NOTIFICATION_CART_EMPTY"] = "Cart is empty!",
						["TPShop_UI_NOTIFICATION_CART_PURCHASED"] = "Successfully purchased {0} items from cart!",
					}, this);
		}
        ...
        {
            Interface.Oxide.DataFileSystem.WriteObject("TPSystem/TPShop/Players", _playerThemeSelect);
            Interface.Oxide.DataFileSystem.WriteObject("TPSystem/TPShop/Player_Carts", _playerCarts);
		}
        private readonly Dictionary<string, ItemDisplayName> _itemName = new Dictionary<string, ItemDisplayName>();
        
		[PluginReference] private readonly Plugin ImageLibrary, Economics, ServerRewards, IQEconomic, TPEconomic, Kits, IQKits, Battles, Duel, ItemCostCalculator, NoEscape, TPMenuSystem;
